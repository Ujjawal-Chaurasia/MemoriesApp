...
export const signup=async(req,res)=>{
    ...
    try {
        ...
        if(password!=confirmPassword)  return res.status(400).json({message:"passoerd didnt matched"})
        // const hashedPassword=  await bcrypt.hash(password,12);
        const result = await new User({email,password,name:`${firstName} ${lastName}`})
        result.save()
